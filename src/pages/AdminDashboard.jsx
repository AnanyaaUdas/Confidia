...
const AdminDashboard = () => {
  ...
  const handleAdminLogin = async (e) => {
    ...
    setLoginError("");
    try {
      const { token: t } = await adminLogin(
        loginForm.username,
        loginForm.password
      );
      setToken(t);
  ...
  const handleDeleteConfirmed = async () => {
    ...
    try {
      ...
      setAllCompliments((list) =>
        list.filter((x) => x.id !== confirmTarget.id)
      );
      setStats((s) => ({
        ...s,
        compliments: Math.max(0, s.compliments - 1),
        reported: confirmTarget.reported
          ? Math.max(0, s.reported - 1)
          : s.reported,
      }));
  ...
  const activePoint =
    hovered !== null ? points[hovered] : points[points.length - 2];
    ...
    return (
      ...
      >
        ...
        >
          ...
          />
          {loginError && (
            <p style={{ color: "#e11d48", fontSize: 13, marginBottom: 8 }}>
              {loginError}
            </p>
          )}
          <button
    ...
    <div className="report-row">
      ...
      <div className="report-body">
        ...
        <div className="report-meta">
          {r.reportReason && (
            <span className="report-reason">🚩 {r.reportReason}</span>
          )}
    ...
    return (
      <section className="card moderation">
        <div className="card-head">
          <h2>Reported Posts</h2>
          <span className="subtle">
            Review flagged compliments — approve to keep live, or delete
            permanently.
          </span>
        </div>
        <div className="tabs">
        ...
        <div className="report-list">
          ...
          {tab === "pending"
            ...
            : list.map((r, i) => (
                <ReportRow r={r} key={`${r.id}-${i}`} resolved />
              ))}
        </div>
      </section>
    );
              ...
              <div className="report-row" key={c.id}>
                ...
                <div className="report-body">
                  <div className="report-to">
                    TO: {c.to}{" "}
                    {c.reported && (
                      <span className="inline-flag">🚩 reported</span>
                    )}
                  </div>
    ...
    return (
      <section className="card moderation">
        <div className="card-head">
          <h2>Users</h2>
          <span className="subtle">
            {list.length} registered users from MongoDB.
          </span>
        </div>
        <div className="report-list">
          {list.length === 0 && (
          ...
          {list.map((u) => (
            <div className="report-row" key={u.id}>
              ...
              <div className="report-body">
                ...
                <div className="report-meta">
                  <span className="report-comments">
                    💌 {u.complimentsShared} shared
                  </span>
                  <span className="report-comments">
                    ❤️ {u.reactionsGiven} reactions
                  </span>
                  <span className="report-comments">
                    🔥 {u.dayStreak} day streak
                  </span>
                </div>
              </div>
            </div>
  ...
  const CommentsView = () => (
    <section className="card moderation">
      <div className="card-head">
        <h2>Comments</h2>
        <span className="subtle">
          {stats.comments} total comments across the wall.
        </span>
      </div>
                ...
                style={{
                  width: `${
                    ((c.commentsCount || 0) /
                      Math.max(
                        1,
                        commentLeaderboard[0]?.commentsCount || 1
                      )) *
                    100
                  }%`,
                }}
      ...
      <section className="row row-bottom">
        <div className="card pulse-card full-height">
          <div className="pulse-text">
            ...
            <h3>
              {stats.compliments}{" "}
              <span>compliments · {stats.users} users</span>
            </h3>
            <p>
  ...
  const SettingsView = () => (
    <section className="card settings-card">
      <div className="card-head">
        <h2>Settings</h2>
        <span className="subtle">Demo controls — change admin password in backend env.</span>
      </div>
      <div className="settings-list">
        <label className="settings-row">
          <div>
            ...
            <span>
              New compliments wait for admin approval before appearing on the
              wall.
            </span>
          </div>
          <input type="checkbox" />
        ...
        <label className="settings-row">
          <div>
            ...
            <span>
              Automatically move a post into the queue once 3 people report it.
            </span>
          </div>
